...
void NumiDetectorConstruction::ConstructHadronAbsorber()
{
  ...
  G4double in = .0254*CLHEP::m;
  //G4double ft = 12.*(CLHEP::cm * 2.54);
  G4double CShld_length = 72*(CLHEP::cm * 2.54);

  // Blue block dimensions, all lengths in Geant4 are half-lengths
  G4double bb_length = 52.30*(CLHEP::cm * 2.54); // modified length of the BluBlocks
  G4double bb_width = 52*(CLHEP::cm * 2.54);
  G4double bb_height = 26*(CLHEP::cm * 2.54);

  // Core Aluminum and Steel block dimensions
  G4double Al_width = 51*(CLHEP::cm * 2.54);
  G4double Al_height = 51*(CLHEP::cm * 2.54);
  G4double Al_length = 12*(CLHEP::cm * 2.54);
  G4double Stl_width = 51*(CLHEP::cm * 2.54);
  G4double Stl_height = 51*(CLHEP::cm * 2.54);
  G4double Stl_length = 9.1*(CLHEP::cm * 2.54);

  // Concrete Shielding block dimensions
  G4double A_width = 1.5*12*(CLHEP::cm * 2.54);
  G4double A_height = 7.5*12*(CLHEP::cm * 2.54);
  G4double A_length = 3*12*(CLHEP::cm * 2.54);
  G4double B_width = 3*12*(CLHEP::cm * 2.54);
  G4double B_height = 7.5*12*(CLHEP::cm * 2.54);
  G4double B_length = 3*12*(CLHEP::cm * 2.54);
  G4double C_width = 3*12*(CLHEP::cm * 2.54);
  G4double C_height = 3*12*(CLHEP::cm * 2.54);
  G4double C_length = 6*12*(CLHEP::cm * 2.54);
  G4double D_width = 1.5*12*(CLHEP::cm * 2.54);
  G4double D_height = 3*12*(CLHEP::cm * 2.54);
  G4double D_length = 6*12*(CLHEP::cm * 2.54);
  G4double J_width = 12*12*(CLHEP::cm * 2.54);
  G4double J_height = 1.5*12*(CLHEP::cm * 2.54);
  G4double J_length = 3*12*(CLHEP::cm * 2.54);
  ...
  G4double StlSlb_width = (5/8.0)*(CLHEP::cm * 2.54);
  G4double StlSlb_height = 50*(CLHEP::cm * 2.54);
  G4double StlSlb_length = 108*(CLHEP::cm * 2.54);

  G4double StlShm_length = 208*(CLHEP::cm * 2.54);
  ...
  // (DP_end + 10.25*12*(CLHEP::cm * 2.54), 0) is now (DP_end + 10.25*12*(CLHEP::cm * 2.54), -1*(CLHEP::cm * 2.54))
  // in the original coordinate frame. 

  G4double xo = DP_end + 10.25*12*(CLHEP::cm * 2.54);
  G4double yo = -1*(CLHEP::cm * 2.54); 
  G4double xp = NumiData->HadrBox_length/2.0 - 6.0625*12*(CLHEP::cm * 2.54);
  G4double yp = 0;
  ...
  xp = (-6.0625*12*(CLHEP::cm * 2.54) - CShld_length/2.0);
  yp = 0;
  ...
  G4Box* Alhole_solid = new G4Box("hole1", 7.5*(CLHEP::cm * 2.54)/2.0, 42.75*(CLHEP::cm * 2.54)/2.0, Al_length/2.0);
  G4Box* Stlhole_solid = new G4Box("hole2", 4.5*(CLHEP::cm * 2.54)/2.0, 42.75*(CLHEP::cm * 2.54)/2.0, Stl_length/2.0);
  ...
  G4ThreeVector AlholePosL = G4ThreeVector(-Stl_width/2.0 + 7.5*(CLHEP::cm * 2.54)/2.0, -1.875*(CLHEP::cm * 2.54), 0);
  G4ThreeVector AlholePosR = G4ThreeVector(Stl_width/2.0 - 7.5*(CLHEP::cm * 2.54)/2.0, -1.875*(CLHEP::cm * 2.54), 0);
  G4ThreeVector StlholePosL = G4ThreeVector(-22.0*(CLHEP::cm * 2.54), -1.875*(CLHEP::cm * 2.54), 0);
  G4ThreeVector StlholePosR = G4ThreeVector(22.0*(CLHEP::cm * 2.54), -1.875*(CLHEP::cm * 2.54), 0);
  G4ThreeVector blockPos;

  G4double CoreX_pos = 0;
  G4double CoreY_pos = 1*(CLHEP::cm * 2.54); // the carriers plate is 2" instead of 1" 
  G4double CoreZ_pos = 0;
  G4double core_offset = 12.75*(CLHEP::cm * 2.54); // This is the offset between the beginning edge of the blublocks and 
  ...
  for (G4int ii=0;ii<8;ii++){
    ...
    if(ii==0){
      ...
      volName.append(no);
      G4Box *stl_slab1_s = new G4Box(volName, 52*(CLHEP::cm * 2.54)/2.0, 2*(CLHEP::cm * 2.54)/2.0, 195.38/2.0*(CLHEP::cm * 2.54));
      volName = "stl_slab1_lv";
      volName.append(no);
      G4LogicalVolume *stl_slab1_lv = new G4LogicalVolume(stl_slab1_s, stl, volName, 0, 0, 0);
      volName = "stl_slab1";
      G4ThreeVector slabpos1 = G4ThreeVector(0, -Al_height/2.0 + CoreY_pos - 2*(CLHEP::cm * 2.54)/2.0, -NumiData->HadrBox_length/2.0 + core_offset + 195.38*(CLHEP::cm * 2.54)/2.0);
      new G4PVPlacement(0, slabpos1, volName, stl_slab1_lv, HadrBox, false, 0);

      volName = "stl_slab2_s";
      volName.append(no);
      G4Box *stl_slab2_s = new G4Box(volName, 52*(CLHEP::cm * 2.54)/2.0, 3.0*(CLHEP::cm * 2.54)/2.0, StlShm_length/2.0);
      volName = "stl_slab2_lv";
      G4LogicalVolume *stl_slab2_lv = new G4LogicalVolume(stl_slab2_s, stl, volName, 0, 0, 0);
      volName = "stl_slab2";
      volName.append(no);
      G4ThreeVector slabpos2 = G4ThreeVector(-bb_width, -bb_height*2.0 + 3.0*(CLHEP::cm * 2.54)/2.0 - 1.0*(CLHEP::cm * 2.54) - Al_height/2, -NumiData->HadrBox_length/2.0 + 2*bb_length);
      new G4PVPlacement(0, slabpos2, volName, stl_slab2_lv, HadrBox, false, 0);

      volName = "stl_slab3_s";
      G4Box *stl_slab3_s = new G4Box(volName, 52*(CLHEP::cm * 2.54)/2.0, 3.0*(CLHEP::cm * 2.54)/2.0, StlShm_length/2.0);
      volName = "stl_slab3_lv";
      volName.append(no);
      G4LogicalVolume *stl_slab3_lv = new G4LogicalVolume(stl_slab3_s, stl, volName, 0, 0, 0);
      volName = "stl_slab3";
      G4ThreeVector slabpos3 = G4ThreeVector(bb_width, -bb_height*2.0 + 3.0*(CLHEP::cm * 2.54)/2.0 - 1.0*(CLHEP::cm * 2.54) - Al_height/2.0, -NumiData->HadrBox_length/2.0 + 2*bb_length);
      new G4PVPlacement(0, slabpos3, volName, stl_slab3_lv, HadrBox, false, 0);

      volName = "stl_slab4_s";
      volName.append(no);
      G4Box *stl_slab4_s = new G4Box(volName, 3*52*(CLHEP::cm * 2.54)/2.0, in/2.0, StlShm_length/2.0);
      volName = "stl_slab4_lv";
      volName.append(no);
      G4LogicalVolume *stl_slab4_lv = new G4LogicalVolume(stl_slab4_s, stl, volName, 0, 0, 0);
      volName = "stl_slab4";
      G4ThreeVector slabpos4 = G4ThreeVector(0, Al_height/2.0 + 3.5*(CLHEP::cm * 2.54), -NumiData->HadrBox_length/2.0 + 2*bb_length);
      ...
      volName = "stl_slab5_s";
      volName.append(no);
      G4Box *stl_slab5_s = new G4Box(volName, 52*(CLHEP::cm * 2.54)/2.0, 2.0*(CLHEP::cm * 2.54)/2.0, 10.0*Stl_length/2.0);
      volName = "stl_slab5_lv";
      volName.append(no);
      G4LogicalVolume *stl_slab5_lv = new G4LogicalVolume(stl_slab5_s, stl, volName, 0, 0, 0);
      volName = "stl_slab5";
      G4ThreeVector slabpos5 = G4ThreeVector(0, Al_height/2.0 + CoreY_pos + 2.0*(CLHEP::cm * 2.54)/2.0, -NumiData->HadrBox_length/2.0 + core_offset + 8*Al_length + 10*Stl_length/2.0);
  ...
  for (G4int ii=0;ii<8;ii++){
    bbx_pos = -bb_width;
    bby_pos = ii/4*bb_height - Al_height/2.0 - bb_height*1.5 + 3.0*(CLHEP::cm * 2.54) - 1.0*(CLHEP::cm * 2.54);
    bbz_pos = ii%4*bb_length-NumiData->HadrBox_length/2.0 + bb_length/2.0;
    sprintf(no, "%d", ii+33);
    volName = "blu_sBLK"; 
    volName.append(no);
  ...
  for (G4int ii=0;ii<8;ii++){
    bbx_pos = bb_width/2.0 + bb_height/2.0 + (ii%2)*bb_height + 1/8.0*(CLHEP::cm * 2.54);
    bby_pos = 2.0*(CLHEP::cm * 2.54) + (bb_width - Al_height)/2.0;
  ...
  for (G4int ii=0;ii<24;ii++){
    bbx_pos = ii%3*(-bb_width) + bb_width;
    bby_pos = ii/12*bb_height + (4.0*(CLHEP::cm * 2.54) + Al_height/2.0 + bb_height/2.0);
    bbz_pos = (ii%12)/3*(bb_length) - NumiData->HadrBox_length/2.0 + bb_length/2.0; 
    sprintf(no, "%d", ii+65);
    volName = "blu_sBLK"; 
    volName.append(no);
  ...
  G4double conc_width = .9144*CLHEP::m;
  G4double conc_length = 7.0*.9144*CLHEP::m;
  G4double conc_height = 2*7.5*12*(CLHEP::cm * 2.54);
  ...
  blockPos = G4ThreeVector(81*(CLHEP::cm * 2.54) + conc_width/2.0, -in - Al_height/2.0 - 4*bb_height + conc_height/2.0, -NumiData->HadrBox_length/2.0+conc_length/2.0);
  new G4PVPlacement(0, blockPos, "conc_BLK", BLK_log[90], HadrBox, false, 0);

  BLK_solid[91] =  new G4Box("conc_sBLK", conc_width/2.0, conc_height/2.0, conc_length/2.0-.9144/2.0*CLHEP::m);
  BLK_log[91] = new G4LogicalVolume(BLK_solid[91], concrete, "conc_lvBLK", 0, 0, 0);
  blockPos = G4ThreeVector(-81*(CLHEP::cm * 2.54) - conc_width/2.0, -in - Al_height/2.0 - 4*bb_height + conc_height/2.0, -NumiData->HadrBox_length/2.0+(conc_length-.9144*CLHEP::m)/2.0);
  new G4PVPlacement(0, blockPos, "conc_BLK", BLK_log[91], HadrBox, false, 0);

  BLK_solid[92] =  new G4Box("conc_sBLK", conc_length/2.0-2*.9144/2.0*CLHEP::m, conc_height/2.0, conc_width/2.0);
  BLK_log[92] = new G4LogicalVolume(BLK_solid[92], concrete, "conc_lvBLK", 0, 0, 0);
  blockPos = G4ThreeVector(-81*(CLHEP::cm * 2.54) - conc_width/2.0 + (conc_length-2*.9144*CLHEP::m)/2.0, -in-Al_height/2.0-4*bb_height+conc_height/2.0, -NumiData->HadrBox_length/2.0 + conc_length + 10*(CLHEP::cm * 2.54) - conc_width/2.0);
  new G4PVPlacement(0, blockPos, "conc_BLK", BLK_log[92], HadrBox, false, 0);

  BLK_solid[93] =  new G4Box("topstl1_sBLK", 3*bb_width/2.0, 18.22*(CLHEP::cm * 2.54)/2.0, 4*bb_width/2.0);
  BLK_log[93] = new G4LogicalVolume(BLK_solid[93], stl, "topstl1_lvBLK", 0, 0, 0);
  blockPos = G4ThreeVector(0, 4*(CLHEP::cm * 2.54) + Al_height/2.0 + bb_height*2.0 + 18.22*(CLHEP::cm * 2.54)/2.0, -NumiData->HadrBox_length/2.0 + 4*bb_width/2.0);
  new G4PVPlacement(0, blockPos, "topstl1_BLK", BLK_log[93], HadrBox, false, 0);

  BLK_solid[94] =  new G4Box("topstl2_sBLK", conc_width/2.0, 18.22*(CLHEP::cm * 2.54)/2.0, conc_length/2.0);
  BLK_log[94] = new G4LogicalVolume(BLK_solid[94], stl, "topstl2_lvBLK", 0, 0, 0);
  blockPos = G4ThreeVector(-81*(CLHEP::cm * 2.54) - conc_width/2.0, -in - Al_height/2.0-4*bb_height+conc_height+18.22*(CLHEP::cm * 2.54)/2.0, -NumiData->HadrBox_length/2.0 + conc_length/2.0);
  new G4PVPlacement(0, blockPos, "topstl2_BLK", BLK_log[94], HadrBox, false, 0);

  BLK_solid[95] =  new G4Box("topstl3_sBLK", conc_width/2.0, 18.22*(CLHEP::cm * 2.54)/2.0, conc_length/2.0);
  ...
  BLK_solid[96] =  new G4Box("topstl4_sBLK", 3*bb_width/2.0, 18.22*(CLHEP::cm * 2.54)/2.0, 2*30*(CLHEP::cm * 2.54)/2.0);
  BLK_log[96] = new G4LogicalVolume(BLK_solid[96], stl, "topstl4_lvBLK", 0, 0, 0);
  blockPos = G4ThreeVector(0, -NumiData->HadrBox_height/2.0 + conc_height + 18.22*(CLHEP::cm * 2.54)/2.0, -NumiData->HadrBox_length/2.0 + 4*bb_length + 2*30*(CLHEP::cm * 2.54)/2.0);
  ...
  BLK_solid[97] =  new G4Box("topstl5_sBLK", 2*9.1*(CLHEP::cm * 2.54)/2.0, 30*(CLHEP::cm * 2.54)/2.0, 4*bb_width/2.0);
  BLK_log[97] = new G4LogicalVolume(BLK_solid[97], stl, "topstl5_lvBLK", 0, 0, 0);
  blockPos = G4ThreeVector(-3*bb_width/2.0 - 2*9.1*(CLHEP::cm * 2.54)/2.0, -NumiData->HadrBox_height/2.0 + conc_height + 18.22*(CLHEP::cm * 2.54) + 30*(CLHEP::cm * 2.54)/2.0, -NumiData->HadrBox_length/2.0 + 4*bb_length/2.0);
  new G4PVPlacement(0, blockPos, "topstl5_BLK", BLK_log[97], HadrBox, false, 0);

  BLK_solid[98] =  new G4Box("topstl6_sBLK", 2*9.1*(CLHEP::cm * 2.54)/2.0, 30*(CLHEP::cm * 2.54)/2.0, 4*bb_width/2.0);
  ...
  BLK_solid[99] =  new G4Box("topstl7_sBLK", 3*bb_width/2.0 + 2*18.22, 30*(CLHEP::cm * 2.54)/2.0, 2*9.1*(CLHEP::cm * 2.54)/2.0);
  BLK_log[99] = new G4LogicalVolume(BLK_solid[99], stl, "topstl7_lvBLK", 0, 0, 0);
  blockPos = G4ThreeVector(0, -NumiData->HadrBox_height/2.0 + conc_height + 18.22*(CLHEP::cm * 2.54) + 30*(CLHEP::cm * 2.54)/2.0, -NumiData->HadrBox_length/2.0 + 4*bb_length + 2*9.1*(CLHEP::cm * 2.54)/2.0);
  ...
  blockPos = G4ThreeVector(0, -NumiData->HadrBox_height/2.0+2*B_height+J_height+3*(CLHEP::cm * 2.54), -CShld_length/2.0+D_length/2.0);
  ...
  G4Box *CShld_stl_solid = new G4Box("CShld_stl_sBLK", 3*bb_width/2.0, 18.22*(CLHEP::cm * 2.54)/2.0, 2*J_length/2.0);
  G4LogicalVolume *CShld_stl_log = new G4LogicalVolume(CShld_stl_solid, stl, "CShld_stl_lvBLK", 0, 0, 0);
  blockPos = G4ThreeVector(0, -NumiData->HadrBox_height/2.0 + 2*B_height + 2*J_height + 3*(CLHEP::cm * 2.54) + 18.22*(CLHEP::cm * 2.54)/2.0, -CShld_length/2.0 + D_length/2.0);
  ...
  CShld_solid[15] = new G4Box("air_sBLK15", 1.5*12*(CLHEP::cm * 2.54)/2.0, 1.5*12*(CLHEP::cm * 2.54)/2.0, 1.5*12*(CLHEP::cm * 2.54)/2.0);
  CShld_log[15] = new G4LogicalVolume(CShld_solid[15], Air, "air_lvBLK15", 0, 0, 0);
  blockPos = G4ThreeVector((A_width+B_width)/2.0 - 1.5*12*(CLHEP::cm * 2.54)/2.0, A_height/2.0 - 1.5*12*(CLHEP::cm * 2.54)/2.0, -2*A_length/2.0 + 1.5*12*(CLHEP::cm * 2.54)/2.0);
