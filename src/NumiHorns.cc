...
void NumiDetectorConstruction::ConstructHorns()
{
  ...
  //Horns
	...
	rmax[ii]=NumiData->PhornROCout[jj];
      ...
      Horn_PM[jj]=new G4Polycone(vol_name, 0., 360.*CLHEP::deg, NumiData->PhornNpoint[jj]+1, z, rmin, rmax);
	...
	rmax[ii]=phornRgivenZ(NumiData->PhornAout[jj],NumiData->PhornBout[jj],NumiData->PhornCout[jj],z[ii]+NumiData->PhornZ1[jj]);
      ...
      G4Polycone* Horn_in=new G4Polycone("Horn_in", 0., 360.*CLHEP::deg, NumiData->PhornNpoint[jj]+1, z, rmin, rmax);      material=GetMaterial(NumiData->PhornGEANTmat[jj]);
      ...
      // Outer conductor
      G4Tubs* Horn_out=new 
      	G4Tubs("Horn_out",NumiData->PhornROCin[jj],NumiData->PhornROCout[jj],(NumiData->PhornZ2[jj]-NumiData->PhornZ1[jj])/2.,0.,360.*CLHEP::deg);
	...
	rmax[kk]=NumiData->PhornROCin[jj];
      ...
      G4Polycone* Horn_inside=new G4Polycone("Horn_inside", 0., 360.*CLHEP::deg, NumiData->PhornNpoint[jj]+1, z, rmin, rmax);
...
vol_name="PF0";
	...
	G4Tubs* Horn_front_cover=new 
	  G4Tubs("Horn_front_cover",rin,rout,(NumiData->PhornThickFront[jj])/2.,0.,360.*CLHEP::deg);
	material=GetMaterial(NumiData->PhornGEANTmat[jj]);
	...
	G4Tubs* Horn_end_cover=new 
	  G4Tubs("Horn_end_cover",rin,rout,(NumiData->PhornThickEnd[jj])/2.,0.,360.*CLHEP::deg);
	material=GetMaterial(NumiData->PhornGEANTmat[jj]);
